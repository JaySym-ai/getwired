...
const FIXTURE_PATH = resolve(__dirname, "../fixtures/fake-opencode.mjs");
...
function buildFakeOpenCodeCommand(): string {
...
async function installFakeOpenCodeBinary(binDir: string): Promise<void> {
  const scriptPath = join(binDir, "opencode");
  await writeFile(scriptPath, buildFakeOpenCodeCommand());
  await chmod(scriptPath, 0o755);
}

async function createTestContext(): Promise<{ cleanup: () => Promise<void>; context: TestContext }> {
  const rootDir = await mkdtemp(join(tmpdir(), "opencode-provider-test-"));
...
async function withFakeOpenCodeMode<T>(mode: FixtureMode, run: () => Promise<T>): Promise<T> {
  const originalPath = process.env.PATH;
  const originalMode = process.env.FAKE_OPENCODE_MODE;
  const originalProviderMode = process.env.OPENCODE_FAKE_MODE;
  const binDir = await mkdtemp(join(tmpdir(), "fake-opencode-bin-"));
  await installFakeOpenCodeBinary(binDir);
  process.env.PATH = [binDir, originalPath].filter(Boolean).join(delimiter);
  process.env.FAKE_OPENCODE_MODE = mode;
  process.env.OPENCODE_FAKE_MODE = mode;
  try {
    return await run();
  } finally {
    if (originalPath === undefined) delete process.env.PATH;
    else process.env.PATH = originalPath;
    if (originalMode === undefined) delete process.env.FAKE_OPENCODE_MODE;
    else process.env.FAKE_OPENCODE_MODE = originalMode;
    if (originalProviderMode === undefined) delete process.env.OPENCODE_FAKE_MODE;
    else process.env.OPENCODE_FAKE_MODE = originalProviderMode;
...
async function collectStream(
  mode: FixtureMode,
  provider: OpenCodeProvider = new OpenCodeProvider(),
): Promise<StreamChunk[]> {
  return withFakeOpenCodeMode(mode, async () => {
...
describe("OpenCodeProvider stderr isolation", () => {
  ...
  it("stdout content is yielded as parsed text chunks", async () => {
    ...
    assert.ok(!output.includes('{"type":"text"'), "expected parsed text rather than raw OpenCode JSON");
  });
  ...
  it("large stderr output is bounded by STDERR_CAP", async () => {
    ...
    const output = joinText(chunks);
    const diagnostic = getTextChunks(chunks).find((chunk) => chunk.includes("[opencode exited with code 23]"));
    assert.match(output, /Analyzing page structure at localhost:3000\.\.\./);
    assert.ok(diagnostic, "expected non-zero exit diagnostics");
    assert.ok(diagnostic.startsWith("\n[opencode exited with code 23]\n"));
