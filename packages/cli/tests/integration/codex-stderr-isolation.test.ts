...
const FIXTURE_PATH = resolve(__dirname, "../fixtures/fake-codex.mjs");
...
function buildFakeCodexCommand(): string {
...
async function installFakeCodexBinary(binDir: string): Promise<void> {
  const scriptPath = join(binDir, "codex");
  await writeFile(scriptPath, buildFakeCodexCommand());
  await chmod(scriptPath, 0o755);
}

async function createTestContext(): Promise<{ cleanup: () => Promise<void>; context: TestContext }> {
  const rootDir = await mkdtemp(join(tmpdir(), "codex-provider-test-"));
...
async function withFakeCodexMode<T>(mode: FixtureMode, run: () => Promise<T>): Promise<T> {
  const originalPath = process.env.PATH;
  const originalMode = process.env.FAKE_CODEX_MODE;
  const binDir = await mkdtemp(join(tmpdir(), "fake-codex-bin-"));
  await installFakeCodexBinary(binDir);
...
async function collectStream(mode: FixtureMode, provider: CodexProvider = new CodexProvider()): Promise<StreamChunk[]> {
  return withFakeCodexMode(mode, async () => {
...
describe("CodexProvider stderr isolation", () => {
  ...
  it("stdout content is yielded as parsed text chunks", async () => {
    ...
    assert.ok(!output.includes('"type":"item.completed"'), "expected parsed text rather than raw Codex JSONL");
  });
  ...
  it("large stderr output is bounded by STDERR_CAP", async () => {
    ...
    const output = joinText(chunks);
    const diagnostic = getTextChunks(chunks).find((chunk) => chunk.includes("[codex exited with code 23]"));
    assert.match(output, /Analyzing page structure at localhost:3000\.\.\./);
